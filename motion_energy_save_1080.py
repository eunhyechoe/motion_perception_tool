...
class MotionEnergy():
    ...
    def img_callback(self, img_in, ensure_contiguous=False):
        ...
        if not self.disable_video_scaling:
            ...
            new_width = width_img_in // 2
            new_height = height_img_in // 2
            img_in = cv2.resize(img_in, (1080,1080))            
        ...
        if self.video_writer is None:
            height_img, width_img, _ = img_in.shape

            if args.file:
                input_folder = os.path.dirname(args.file)
                output_folder = os.path.join(input_folder, "../output")
                os.makedirs(output_folder, exist_ok=True) 
                output_filename = os.path.join(output_folder, os.path.basename(args.file).split('.')[0] + '_output.mp4')
            else:
                output_filename = 'motion_energy_output.mp4'
            
            self.video_writer = cv2.VideoWriter(
                    output_filename,  
                    cv2.VideoWriter_fourcc(*'mp4v'),  # codec
                    60,  # FPS
                    (width_img, height_img)  # frame size
            )

        # Save combined video (NEW)
        if self.combined_writer is None:
            output_combined_folder = os.path.join(input_folder, "../output/combined")
            os.makedirs(output_combined_folder, exist_ok=True) 
            output_combined_filename = os.path.join(output_combined_folder, os.path.basename(args.file).split('.')[0] + '_combined.mp4')
            self.combined_writer = cv2.VideoWriter(
                output_combined_filename, 
                cv2.VideoWriter_fourcc(*'mp4v'),  
                60,  # FPS
                (width_img * 2, height_img)  # combined frame size (double the width)
            )
        ...
        if len(self.img_grey_stack) == self.num_frames:
            ...
            if self.video_writer is not None:
                self.video_writer.write(motion_visu_img)

            # Save combined video (NEW)
            combined_frame = np.hstack((img_in, motion_visu_img))
            if self.combined_writer is not None:
